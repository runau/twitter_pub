...
def main():
    ...
    doneList = requests.get(
        f'https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name={SCREEN_NAME}&count=20',
        auth=getOauth()
    ).json()
    ...
    response = requests.get(
        f'https://api.twitter.com/1.1/search/tweets.json?q={search}&count=20&lang=ja&result_type=mixed&tweet_mode=extended',
        auth=getOauth()
    ).json()
    ...
    tweetList = getParam("tweetList") + \
        list(map(lambda x: x['id_str'], statusList))
    ...
    for status in statusList:
        ...
        repResponse = requests.get(
            f'https://api.twitter.com/1.1/search/tweets.json?q={search}&count=30&lang=ja&result_type=mixed',
            auth=getOauth()
        ).json()
